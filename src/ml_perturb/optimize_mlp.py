...
def run_optimization(feature_set):
    ...
    X_train_scaled, y_train, X_val_scaled, y_val = load_data(feature_set)
    ...
    out_path = os.path.join("outputs_perturb", f"mlp_{feature_set}_best_params.json")
    ...
    try:
        ...
        fig_hist.write_image(os.path.join(plots_dir, f"mlp_opt_history_{feature_set}.pdf"))
        
        fig_para = vis.plot_parallel_coordinate(study)
        fig_para.write_image(os.path.join(plots_dir, f"mlp_parallel_coordinate_{feature_set}.pdf"))
    except Exception as e:
        logger.warning(f"Could not save visualizations: {e}")

if __name__ == "__main__":
    logger.info("=== Optimizing MLP for M-R (Mass, Radius) ===")
    run_optimization('MR')
    logger.info("=== Optimizing MLP for M-R-L (Mass, Radius, Lambda) ===")
