...
def run_confusion_matrix():
    ...
    for fset in ["MR", "MRL"]:
        ...
        if os.path.exists(xgb_model_path):
            ...
            plot_path_xgb = os.path.join(plots_dir, f"confusion_matrix_xgboost_{fset}.pdf")
            plot_confusion_matrices(y_test, xgb_preds, f"XGBoost ({fset})", plot_path_xgb)
            logger.info(f"[{fset}] Saved XGBoost Confusion Matrix to {plot_path_xgb}")
        else:
        ...
        if os.path.exists(mlp_params_path) and os.path.exists(mlp_model_path):
            ...
            plot_path_mlp = os.path.join(plots_dir, f"confusion_matrix_mlp_{fset}.pdf")
            plot_confusion_matrices(y_test, mlp_preds, f"MLP ({fset})", plot_path_mlp)
            logger.info(f"[{fset}] Saved MLP Confusion Matrix to {plot_path_mlp}")
        else:
