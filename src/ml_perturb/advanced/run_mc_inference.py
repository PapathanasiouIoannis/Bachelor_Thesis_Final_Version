...
from src.ml_perturb.advanced.common import (
    FEATURE_SETS,
    load_perturb_mlp,
    load_perturb_scaler,
    load_perturb_xgb,
...
def run_mc_inference():
    logger.info("Initializing synthetic score sampling for the perturbed pipeline...")
    ...
    x_mc_scaled_3cols = pd.DataFrame(
        scaler.transform(x_mc_3cols), columns=x_mc_3cols.columns
    )
    ...
    for fset, features in FEATURE_SETS.items():
        ...
        mlp_model = load_perturb_mlp(
            fset, input_dim=x_mc_scaled.shape[1], device=device
        )
        with torch.no_grad():
