...
from src.ml.advanced.common import (
    clean_plot_dir,
    load_clean_mlp,
    load_clean_scaler,
    load_clean_xgb,
...
def run_mc_inference():
    logger.info("Initializing restricted synthetic score sampling (XGBoost vs MLP)...")

    n_samples = 1000 if fast_enabled() else 10000
    m_obs, m_err = 1.4, 0.05
    r_obs, r_err = 11.5, 0.5
    logger.info(
        f"Generating {n_samples} MC samples for M={m_obs}+/-{m_err} M_sun, R={r_obs}+/-{r_err} km"
    )
    ...
    x_mc = pd.DataFrame(
        {"Mass": mass_samples, "Radius": radius_samples, "log10_Lambda": lambda_samples}
    )
    ...
    logger.info(
        f"[XGBoost] Expected score: {xgb_expected:.4f} [95% interval: {xgb_lb:.4f}, {xgb_ub:.4f}]"
    )
    logger.info(
        f"[MLP] Expected score: {mlp_expected:.4f} [95% interval: {mlp_lb:.4f}, {mlp_ub:.4f}]"
    )
    ...
    ax1.errorbar(
        ...
        capsize=3,
        linewidth=1.5,
        label="Selected mean +/- 1 sigma",
    )
    ax1.set_xlabel(r"Mass ($M_\odot$)")
    ax1.set_ylabel("Radius (km)")
    ax1.set_title(
        f"XGBoost synthetic scores\nMean: {xgb_expected:.1%} [central 95% range: {xgb_lb:.1%} - {xgb_ub:.1%}]"
    )
    ...
    ax2.set_title(
        f"MLP synthetic scores\nMean: {mlp_expected:.1%} [central 95% range: {mlp_lb:.1%} - {mlp_ub:.1%}]"
    )
    ax2.grid(True, alpha=0.3)
    ax2.legend(loc="upper right")
    fig.colorbar(scatter2, ax=ax2, label="Uncalibrated CFL4 model score")

    plt.suptitle(f"Restricted synthetic score sampling (N={n_samples})", fontsize=16)
    plt.tight_layout()
