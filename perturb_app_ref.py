...
# -----------------------------------------
st.set_page_config(
    page_title="Compact-star synthetic classifier — perturbation-sensitivity demonstration",
    layout="wide",
...
st.caption(
    "Perturbation-sensitivity demonstration · compactness excluded from the classifier features"
)
st.info(
    "This independent post-thesis demonstration explores classifier-score sensitivity inside a "
    "restricted synthetic, perturbed model comparison. Its outputs are not observational "
    "measurements, calibrated astrophysical probabilities, composition posteriors, or evidence "
    "about the interior of any real star."
)
with st.expander("Scope, assumptions, and responsibility"):
    st.markdown(
        """
- The demonstration is tied to the repository's retained synthetic hadronic-surrogate and
  fixed-CFL4 experiment; it is not a universal hadronic-versus-quark classifier.
- The perturbed training data use a simplified synthetic error law. Input uncertainty here is
  represented by independent Gaussian samples rather than real NICER or LIGO posterior samples.
...
def load_xgboost(feature_set):
    model_path = (
        PATHS.outputs_perturb_root / f"xgboost_{feature_set}" / "xgboost_weights.json"
    )
...
@st.cache_data(show_spinner="Loading perturbed synthetic model space...")
def load_background_manifold(_scaler):
    ...
    df_raw["Model_Class"] = np.where(
        full_df["Label"] == 1,
        "perturbed fixed-CFL4 benchmark",
        "perturbed hadronic surrogate",
    )
...
if not os.path.exists(PATHS.perturb_tensor_dir / "scaler_perturb.joblib"):
    st.error(
        "🚨 **Fatal Error:** Could not find `scaler_perturb.joblib`. Please ensure the Perturbed ML pipeline has been run."
    )
    st.stop()
...
with st.sidebar:
    ...
    obs_mode = st.radio(
        ...
        ["Mass–radius inputs", "Mass–radius–tidal inputs"],
        help="Mass–radius mode uses the MR models. The joint mode uses the MRL models.",
    )

    # Map the visible input mode to the retained feature set.
    feature_set = "MR" if obs_mode == "Mass–radius inputs" else "MRL"
    ...
    R_err = st.number_input(
        "Radius Gaussian σ (km)", min_value=0.01, max_value=3.0, value=0.5, step=0.05
    )
...
if run_btn:
    ...
    with st.spinner(
        f"Sampling the perturbed synthetic classifier with {model_choice} [{feature_set}]..."
    ):
        N = 5000
        ...
        if feature_set == "MRL":
            ...
            X_mc = pd.DataFrame({"Mass": mass_samples, "Radius": radius_samples})
        ...
        # -----------------------------------------
        st.markdown("## Perturbed synthetic model space")
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    color=np.where(
                        bg_manifold["Model_Class"] == "perturbed fixed-CFL4 benchmark",
                        "#ff4b4b",
                        "#1f77b4",
                    ),
                    size=4,
                ...
                name="Perturbed synthetic training support",
                hoverinfo="skip",
            )
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate="<b>Mass:</b> %{y:.2f}<br><b>Radius:</b> %{x:.2f}<br><b>Model score:</b> %{marker.color:.2%}<extra></extra>",
            )
        )
        ...
        with st.expander("Reference XGBoost score explanation (SHAP)"):
            ...
            if feature_set == "MRL":
                mean_obs_df = pd.DataFrame(
                    {"Mass": [M_obs], "Radius": [R_obs], "log10_Lambda": [L_obs]}
                )
                mean_scaled = pd.DataFrame(
                    scaler.transform(mean_obs_df), columns=mean_obs_df.columns
                )
            else:
                mean_obs_df = pd.DataFrame({"Mass": [M_obs], "Radius": [R_obs]})
                dummy = pd.DataFrame(
                    {"Mass": [M_obs], "Radius": [R_obs], "log10_Lambda": [0.0]}
                )
                scaled_dummy = scaler.transform(dummy)
                mean_scaled = pd.DataFrame(
                    scaled_dummy[:, :2], columns=["Mass", "Radius"]
                )
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"perturbed_synthetic_scores_{model_choice.replace(' ', '_')}_{feature_set}.csv",
            mime="text/csv",
        )
