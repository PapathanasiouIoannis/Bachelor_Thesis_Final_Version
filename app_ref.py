...
def load_mlp():
    ...
    return load_mlp_model(PATHS.outputs_root / "mlp_best_params.json", PATHS.outputs_root / "mlp" / "mlp_weights.pth", 3, device)
    ...
    with st.spinner(f"Running highly-parallelized Monte Carlo Inference using {model_choice}..."):
        ...
        if obs_mode == "Multi-Messenger (Mass, Radius & Λ)":
            ...
            fig.add_trace(go.Scatter3d(
                ...
                mode='markers',
                marker=dict(
                    color=np.where(bg_sample_3d['Phase_Class'] == "Theoretical Quark EoS", '#ff4b4b', '#1f77b4'),
                    size=3,
                    opacity=0.25
                ),
                name="Theoretical EoS Manifold",
